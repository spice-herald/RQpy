...
def calculate_substrate_mass(vol, tm):
    """
    Helper function for calculating the mass of substrate given its volume.

    Parameters
    ----------
    vol : float
        The volume of the substrate that we want to know the mass of, in units of meters^3.
    tm : str, int
    ...
    -------
    mass : float
        The mass, in kg, of the substrate based on the inputted volume and target material.

    """

    rho = mendeleev.element(tm).density / (rp.constants.centi**3 * rp.constants.kilo) # density in kg/m^3
    mass = rho * vol
...
class SensEst(object):
    """
    Class for setting up and running an estimate of the sensitivity of a device, given expected backgrounds.
    ...
    tm : str, int
        The target material of the detector. Can be passed as either the atomic symbol, the
        atomic number, or the full name of the element.
    exposure : float
    ...
    def __init__(self, m_det, time_elapsed, eff=1, tm="Si"):
        ...
        eff : float, optional
            The estimated efficiency due to data selection, live time losses, etc. Default is 1.
        tm : str, int, optional
            The target material of the detector. Can be passed as either the atomic symbol, the
            atomic number, or the full name of the element. Default is 'Si'.
    ...
    def add_flat_bkgd(self, flat_rate):
        ...
        ----------
        flat_rate : float
            The flat background rate, in units of events/kg/kev/day (DRU).
    ...
    def add_noise_bkgd(self, sigma, n_win, fs):
        ...
        n_win : float
            ...
            in the trigger window.
        fs : float
            The digitization rate of the data being used in the trigger algorithm,
            units of Hz.
    ...
    def add_dm_bkgd(self, m_dm, sig0):
        ...
        m_dm : float
            ...
            event rate. Expected units are GeV/c^2.
        sig0 : float
            The dark matter cross section at which to calculate the expected
            differential event rate. Expected units are cm^2.
    ...
    def add_arb_bkgd(self, function):
        ...
        function : FunctionType
            A function that returns a background rate in units of events/kg/keV/day
            when inputted energy, where the energies are in units of keV.
    ...
    def run_sim(self, threshold, e_high, m_dms=None, nexp=1, npts=1000, plot_bkgd=False):
        """
        Method for running the simulation for getting the sensitivity estimate.
        ...
        m_dms : ndarray, optional
            Array of dark matter masses (in GeV/c^2) to run the Optimum Interval code.
            Default is 50 points from 0.05 to 2 GeV/c^2.
        nexp : int, optional
        ...
        -------
        m_dms : ndarray
            The dark matter masses in GeV/c^2 that upper limit was set at.
        sig : ndarray
            The cross section in cm^2 that the upper limit was determined to be.
    ...
    def _generate_background(self, en_interp, plot_bkgd=False):
        ...
        ----------
        en_interp : ndarray
            The energies at which the total simulated background rate will be interpolated,
            in units of keV.
        plot_bkgd : bool, optional
            Option to plot the background being used on top of the generated data, for diagnostic
            purposes. If `nexp` is greater than 1, then only the first generated dataset is plotted.
        ...
        if len(self._backgrounds) == 0:
            raise ValueError(
                "No backgrounds have been added, add some using the methods of SensEst."
            )
        ...
        tot_bkgd_func = lambda x: np.stack([bkgd(x) for bkgd in self._backgrounds], axis=1).sum(axis=1)
    ...
    def _plot_bkgd(self, evts, en_interp, tot_bkgd_func):
        """Hidden Method for plotting the generated events on top of the inputted backgrounds."""

        ratecomp = rp.RatePlot((en_interp.min(), en_interp.max()), figsize=(9, 6))
        ratecomp.add_data(
            evts,
            self.exposure,
        ...
        for item in [ratecomp.ax.title, ratecomp.ax.xaxis.label, ratecomp.ax.yaxis.label]:
